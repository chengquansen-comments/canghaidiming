...
当前 MVP v0.7 使用“收式阶 × 攻守变”规则。

- 收式阶是每张招式牌的承接阶位，范围 `1` 到 `10`。
...
- 玩家确定兵器后武境为 `2`，但初始牌组只包含 `1` 式阶基础牌。
- 武境与收式阶强绑定：武境几，最高可用几阶收式。
- 每完成 `2` 场战斗，玩家武境 `+1`，最高武境为 `10`。
- 战斗中，角色连续打出收式阶严格递增的有效招式时，形成收式连击。
- 连击倍率按指数增长：`1连×1`、`2连×2`、`3连×4`、`4连×8`，之后以此类推。
- 当前 MVP 中，连击倍率只放大当前招式的 `damage` 与 `guard`。
- `break_momentum`、`gain_momentum`、抽牌、位移、耗势、先机、虚招等机制效果暂不受连击倍率影响。
- 变牌可以推进连击，但变牌自身的机制效果不吃倍率。
- 若招式未中、朝向错误、行动被打断或角色崩势，则连击断开。
