...
func _sample_story_encounters(card_catalog: Dictionary) -> Dictionary:
	...
	var encounters := StoryBattleLoader.load_encounters()
...
func _initial_player_profile(role_id: String) -> Dictionary:
	if role_id == "blademaster":
		return {"role": "blademaster", "career": "腰刀武官", "weapon": "腰刀", "max_hp": 31, "hp": 31, "max_posture": 9, "posture": 7, "martial_level": 1, "battles_won": 0}
	return {"role": "spearman", "career": "长枪武官", "weapon": "长枪", "max_hp": 26, "hp": 26, "max_posture": 8, "posture": 5, "martial_level": 1, "battles_won": 0}
...
func _apply_profile_growth(profile: Dictionary, encounter_id: String) -> void:
	...
	profile["max_hp"] = int(profile.get("max_hp", 1)) + int(reward.get("hp_gain", 0))
	profile["max_posture"] = int(profile.get("max_posture", 1)) + int(reward.get("posture_gain", 0))
	profile["martial_level"] = int(profile.get("martial_level", 1)) + int(reward.get("martial_gain", 0))
...
func _print_report(report: Dictionary) -> void:
	print("story battle number report samples=%d seed=%d entries=%d profile=%s role=%s" % [samples, seed, int(report.get("entry_count", 0)), profile_mode, player_role])
