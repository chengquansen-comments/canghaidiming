...
const OUT_PATH := "res://data/design/generated_minimal_playable_round_report.tsv"
...
func _initialize() -> void:
	...
	for node in pool:
		...
		rows.append({
			...
			"notes": "minimal_playable_round_ready",
		})
	...
	_write_tsv(rows)
	print("GENERATED_MINIMAL_PLAYABLE_ROUND_PROBE_DONE")
	quit()
...
func _write_tsv(rows: Array) -> void:
	...
	if f == null:
		return
	f.store_line("node_id\tbattle_slot_id\tselected_in_probe\tbattle_controller_initialized\tbattle_context_loaded\tround_initialized\tplayer_input_ready\tsource\tenemy_deck_id\tenemy_deck_loaded\tenemy_intent_available\tenemy_action_candidate_available\tcard_pool_count\tgenerated_card_pool_available\tcompatible_card_count\treward_plan_id\treward_source\tnarrative_key_count\troute_gate_count\tfallback_policy\tlegacy_fallback_available\twrites_card_data\twrites_story_data\twrites_battle_state\twrites_combat_result\tnotes")
	for rowv in rows:
		if typeof(rowv) != TYPE_DICTIONARY:
			continue
		var row: Dictionary = rowv
		f.store_line("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % [
			str(row.get("node_id", "")),
