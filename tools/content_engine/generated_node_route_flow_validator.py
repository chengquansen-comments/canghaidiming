...
REPORT = Path("data/design/generated_node_route_flow_report.tsv")
...
def main() -> int:
    ...
    generated_rows = [r for r in rows if r.get("is_generated_preview_slot") == "true"]
    if len(generated_rows) != 16:
        return fail(f"report generated slot 数量必须 16，当前 {len(generated_rows)}")
    ...
    for r in generated_rows:
        ...
        if r.get("node_candidate_available") != "true":
            return fail(f"{slot}: node_candidate_available 必须 true")
    ...
    non = non_rows[0]
    if non.get("node_candidate_available") != "false":
        return fail("非 generated preview slot node_candidate_available 必须 false")
    ...
    print("PASS: generated preview slot=16，node candidate 全部可构建")
    print("PASS: battle_flow/enemy_deck/card_pool/operation/narrative/route_gate 指标通过")
    print("PASS: prologue_01 reward=rw_prologue_01，reward 缺失槽位回落 legacy")
    print("PASS: 非 generated preview slot 保持 legacy，未写 story/battle/combat")
    print("RESULT: PASS")
    return 0
