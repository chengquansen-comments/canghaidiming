...
const ADAPTER := preload("res://scripts/generated_playable_battle_entry_adapter.gd")
...
const OUT_PATH := "res://data/design/generated_playable_battle_entry_report.tsv"
...
func _initialize() -> void:
	...
	var fallback_map := _load_reward_fallback_map()
	...
	for node in pool:
		...
		if not selected_any:
			...
			selected_any = true
		var entry: Dictionary = adapter.build_playable_battle_entry_from_node(node_id)
		var ctx: Dictionary = adapter.apply_generated_entry_to_battle_context(entry, {})
		var reward_plan_id := str(entry.get("reward_plan_id", ""))
		var reward_source := str(entry.get("reward_source", "legacy"))
		if bool(fallback_map.get(slot, false)):
			reward_plan_id = ""
		...
		rows.append({
			...
			"playable_entry_available": str(str(entry.get("playable_entry_source", "legacy")) != "legacy").to_lower(),
			"can_start_playable_battle": str(bool(entry.get("can_start_playable_battle", false))).to_lower(),
			"battle_context_initialized": str(bool(entry.get("battle_context_initialized", false)) and bool(ctx.get("generated_battle_context_initialized", false))).to_lower(),
			"playable_entry_source": str(entry.get("playable_entry_source", "legacy")),
			...
			"notes": "generated_narrative_keys=%s;generated_route_gates=%s" % [
				str(ctx.get("generated_narrative_keys", [])),
				str(ctx.get("generated_route_gates", [])),
			],
		})

	var non_entry: Dictionary = adapter.build_playable_battle_entry_from_node(NON_NODE_ID)
	rows.append({
		"node_id": NON_NODE_ID,
		"battle_slot_id": "sample_non_generated_preview_slot",
		"selected_in_probe": "false",
		"playable_entry_available": str(str(non_entry.get("playable_entry_source", "legacy")) != "legacy").to_lower(),
		"can_start_playable_battle": str(bool(non_entry.get("can_start_playable_battle", false))).to_lower(),
		"battle_context_initialized": str(bool(non_entry.get("battle_context_initialized", false))).to_lower(),
		"playable_entry_source": str(non_entry.get("playable_entry_source", "legacy")),
		...
		"generated_card_pool_available": str(bool(non_entry.get("generated_card_pool_available", false))).to_lower(),
	...
	_write_tsv(rows)
	print("GENERATED_PLAYABLE_BATTLE_ENTRY_PROBE_DONE")
	quit()
...
func _write_tsv(rows: Array) -> void:
	...
	if f == null:
		return
	f.store_line("node_id\tbattle_slot_id\tselected_in_probe\tplayable_entry_available\tcan_start_playable_battle\tbattle_context_initialized\tplayable_entry_source\tenemy_deck_id\tenemy_deck_source\tcard_pool_count\tgenerated_card_pool_available\treward_plan_id\treward_source\tnarrative_key_count\troute_gate_count\tfallback_policy\tlegacy_fallback_available\twrites_card_data\twrites_story_data\twrites_battle_state\twrites_combat_result\tnotes")
	for rowv in rows:
		if typeof(rowv) != TYPE_DICTIONARY:
			continue
		var row: Dictionary = rowv
		f.store_line("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % [
			str(row.get("node_id", "")),
			str(row.get("battle_slot_id", "")),
			str(row.get("selected_in_probe", "")),
			str(row.get("playable_entry_available", "")),
			str(row.get("can_start_playable_battle", "")),
			str(row.get("battle_context_initialized", "")),
			str(row.get("playable_entry_source", "")),
