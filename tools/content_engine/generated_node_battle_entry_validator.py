...
REPORT = Path("data/design/generated_node_battle_entry_report.tsv")
...
def main() -> int:
    ...
    for r in generated_rows:
        ...
        if r.get("battle_entry_available") != "true":
            return fail(f"{slot}: battle_entry_available 必须 true")
        if r.get("can_enter_battle") != "true":
            return fail(f"{slot}: can_enter_battle 必须 true")
    ...
    non = non_rows[0]
    if non.get("entry_source") != "legacy":
        return fail("非 generated node entry_source 必须 legacy")

    bad = changed_forbidden_files()
    if bad:
        return fail("检测到禁止修改文件: " + ", ".join(bad))

    print("PASS: generated node count=16，每个 node 可构建 battle_entry")
    print("PASS: can_enter_battle=true，至少 1 个 selected node")
