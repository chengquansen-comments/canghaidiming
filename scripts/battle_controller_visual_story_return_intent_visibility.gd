...
# Enemy intent visibility layer for story-return battles.
# Keep parser-time dependencies local and avoid global class type annotations here.
...
func _intent_bubble_text(card, actor_slot: int, opponent_slot: int, target_slot: int) -> String:
...
func _effect_preview_text() -> String:
	...
	var e_intent = enemy_intent
	var p_card = p_intent.actual_card if p_intent != null else null
	var e_card = e_intent.actual_card if e_intent != null else null
...
func _reactive_threat_preview_text() -> String:
	...
	var player_card = draft_player_intent.actual_card if draft_player_intent != null else null
	var lines: Array[String] = []
	...
	lines.append("我方响应：%s / %s / 伤%d / 势-%d" % [player_card.display_name if player_card != null else "未选招式", _range_text_safe(str(result.get("player_range", "none"))), int(result.get("player_damage", 0)), int(result.get("player_break", 0))])
	if visibility == BattleIntentVisibilityForStoryReturn.VISIBILITY_TYPE:
...
func _redacted_enemy_step_text(step: Dictionary, visibility: String, e_card) -> String:
