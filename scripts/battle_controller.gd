...
func _impact_feedback(flash_color: Color, shake_strength: float = 6.0) -> void:
...
func _resolve_combo_chain_if_any(actor: Fighter, target: Fighter, intent: IntentData) -> Array[String]:
	...
	_impact_feedback(fx.get("start_flash", Color("ffd479")), float(fx.get("start_shake", 4.0)))
	...
	for idx in range(combo.get("followups", []).size()):
		...
		if remaining_damage > 0:
			...
			_impact_feedback(Color("ffc7c7"), 3.0 if not bool(segment.get("is_finisher", false)) else float(fx.get("finisher_shake", 8.0)))
		...
		if bool(segment.get("is_finisher", false)):
			...
			_impact_feedback(fx.get("finisher_flash", Color("ff4d6d")), float(fx.get("finisher_shake", 9.0)))
...
func _status_text() -> String:
	...
	lines.append("- 枪手终结表现偏穿透；刀客终结表现偏斩切")
	if awaiting_player_input:
		lines.append("")
