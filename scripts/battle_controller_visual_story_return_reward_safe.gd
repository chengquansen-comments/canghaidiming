...
# Story battle result reward layer with local parser-time narrative context alias.

const StoryReturnRewardNarrativeContext := preload("res://scripts/narrative_battle_context.gd")
...
func _battle_result_is_proxy_player() -> bool:
	return StoryReturnRewardNarrativeContext.has_request() and not StoryReturnRewardNarrativeContext.should_override_player_profile()

func _battle_result_should_offer_player_reward() -> bool:
	return StoryReturnRewardNarrativeContext.has_request() and not _battle_result_is_proxy_player()

func _battle_result_should_use_story_size() -> bool:
	return StoryReturnRewardNarrativeContext.has_request() and not _battle_result_is_proxy_player()

func _projected_player_growth_text() -> String:
	if not StoryReturnRewardNarrativeContext.has_player_profile():
		...
		var fallback_new_hp: int = StoryReturnRewardNarrativeContext.PLAYER_INITIAL_HP + maxi(0, fallback_next_level - StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MARTIAL_LEVEL) * 2
		var fallback_new_posture: int = clampi(StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MAX_POSTURE + maxi(0, fallback_next_level - StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MARTIAL_LEVEL), StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MAX_POSTURE, StoryReturnRewardNarrativeContext.PLAYER_MAX_POSTURE)
		var fallback_new_qinggong: int = clampi(StoryReturnRewardNarrativeContext.PLAYER_INITIAL_QINGGONG + int(maxi(0, fallback_next_level - StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MARTIAL_LEVEL) / 3), StoryReturnRewardNarrativeContext.PLAYER_INITIAL_QINGGONG, StoryReturnRewardNarrativeContext.PLAYER_MAX_QINGGONG)
	...
	var profile: Dictionary = StoryReturnRewardNarrativeContext.get_player_profile() as Dictionary
	var old_level: int = int(profile.get("martial_level", 1))
	var new_level: int = old_level + 1
	var old_hp: int = int(profile.get("max_hp", StoryReturnRewardNarrativeContext.PLAYER_INITIAL_HP))
	var old_current_hp: int = int(profile.get("hp", old_hp))
	var old_posture: int = int(profile.get("max_posture", StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MAX_POSTURE))
	var old_current_posture: int = int(profile.get("posture", old_posture))
	var old_qinggong: int = int(profile.get("qinggong", StoryReturnRewardNarrativeContext.PLAYER_INITIAL_QINGGONG))
	var old_wins: int = int(profile.get("battles_won", 0))
	var new_hp: int = StoryReturnRewardNarrativeContext.PLAYER_INITIAL_HP + maxi(0, new_level - StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MARTIAL_LEVEL) * 2
	var new_posture: int = clampi(StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MAX_POSTURE + maxi(0, new_level - StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MARTIAL_LEVEL), StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MAX_POSTURE, StoryReturnRewardNarrativeContext.PLAYER_MAX_POSTURE)
	var new_qinggong: int = clampi(StoryReturnRewardNarrativeContext.PLAYER_INITIAL_QINGGONG + int(maxi(0, new_level - StoryReturnRewardNarrativeContext.PLAYER_INITIAL_MARTIAL_LEVEL) / 3), StoryReturnRewardNarrativeContext.PLAYER_INITIAL_QINGGONG, StoryReturnRewardNarrativeContext.PLAYER_MAX_QINGGONG)
...
func _sample_battle_reward_choices(count: int) -> Array[CardData]:
	...
	if StoryReturnRewardNarrativeContext.has_player_profile():
		var card_state: Dictionary = StoryReturnRewardNarrativeContext.get_player_card_state() as Dictionary
