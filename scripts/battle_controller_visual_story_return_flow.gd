extends "res://scripts/battle_controller_visual_story_return_reward.gd"

# Story battle result confirmation and return flow layer.
...
func _on_battle_result_confirm_pressed() -> void:
	...
	if StoryReturnNarrativeBattleContext.has_request():
		...
		var source_scene: String = StoryReturnNarrativeBattleContext.source_scene
		...
		if result_key == "win" and not _selected_battle_reward_card_id.is_empty():
			StoryReturnNarrativeBattleContext.grant_player_cards([_selected_battle_reward_card_id])
		StoryReturnNarrativeBattleContext.set_result(result_key)
