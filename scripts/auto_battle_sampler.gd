...
# Lightweight no-UI Monte Carlo sampler for the current 1v1 spear/blade prototype.
# It intentionally uses plain dictionaries so it can run from debug UI without
# touching the live battle state.
...
static func run_batch(player_cards: Array, enemy_cards: Array, options: Dictionary = {}) -> Dictionary:
	...
	for i in range(sample_count):
		var result := run_single(player_cards, enemy_cards, max_turns, rng)
		_record_result(aggregate, result)
...
static func run_single(player_cards: Array, enemy_cards: Array, max_turns: int, rng: RandomNumberGenerator) -> Dictionary:
	...
	for turn in range(max_turns):
		...
		var snapshot := {
			"player_position": int(p.position),
			"enemy_position": int(e.position),
			"player_facing": str(p.facing),
			"enemy_facing": str(e.facing),
			"player_card": p_card,
			"enemy_card": e_card,
			"order": order,
			"player_momentum": int(p.momentum),
			"enemy_momentum": int(e.momentum),
			"player_guard": int(p.guard),
			"enemy_guard": int(e.guard),
			"player_broken": bool(p.broken),
			"enemy_broken": bool(e.broken)
		}
		var sim := PreviewConsistencyChecker.simulate(snapshot)
		...
		if int(p.hp) <= 0 or int(e.hp) <= 0:
			break
	var winner := "draw"
	if int(p.hp) > 0 and int(e.hp) <= 0:
		winner = "player"
	elif int(e.hp) > 0 and int(p.hp) <= 0:
		winner = "enemy"
	elif int(p.hp) > int(e.hp):
		winner = "player_timeout"
	elif int(e.hp) > int(p.hp):
	...
	return {
		...
		"self_move_count": self_move_count,
...
static func _score_card(card: Dictionary, actor: Dictionary, target: Dictionary) -> float:
	...
	var score := 0.0
	if distance >= int(card.get("min_distance", 0)) and distance <= int(card.get("max_distance", 8)):
		score += 8.0
	else:
		score -= 3.0
	if int(actor.momentum) < int(card.get("momentum_cost", 0)):
		score -= 8.0
	score += float(card.get("damage", 0)) * 0.8
	score += float(card.get("break_momentum", 0)) * 0.7
	score += float(card.get("gain_momentum", 0)) * 0.5
	score += float(card.get("guard", 0)) * 0.35
	if str(actor.get("style", "")) == "spear":
		if int(card.get("target_push_after", 0)) > 0:
			score += 2.5
		if distance == 2 or distance == 3:
			score += 2.0
	else:
		if int(card.get("target_pull_after", 0)) > 0 or int(card.get("self_move_after", 0)) > 0:
			score += 2.5
...
static func _move_toward_preferred_range(actor: Dictionary, target: Dictionary, preferred: Array) -> void:
	...
	if distance > max_pref:
		actor.position = clampi(int(actor.position) + dir, 0, 8)
	elif distance < min_pref:
		actor.position = clampi(int(actor.position) - dir, 0, 8)
	_face_each_other(actor, target)

static func _face_each_other(a: Dictionary, b: Dictionary) -> void:
	if int(b.position) > int(a.position):
		a.facing = "right"
		b.facing = "left"
	elif int(b.position) < int(a.position):
		a.facing = "left"
		b.facing = "right"

static func _movement_count(card: Dictionary, field: String) -> int:
	if str(card.get("move_condition", "none")) == "none":
		return 0
...
static func _finalize_aggregate(aggregate: Dictionary) -> void:
	...
	aggregate.player_win_rate = float(aggregate.player_wins) / float(n)
	aggregate.enemy_win_rate = float(aggregate.enemy_wins) / float(n)
	aggregate.draw_rate = float(aggregate.draws) / float(n)
	aggregate.avg_turns = float(aggregate.turns_total) / float(n)
...
static func format_report(result: Dictionary) -> String:
	if result.is_empty():
		return "未采样"
	var text := ""
	text += "样本:%d  平均回合:%.1f\n" % [int(result.get("sample_count", 0)), float(result.get("avg_turns", 0.0))]
	text += "枪胜:%.1f%%  刀胜:%.1f%%  平:%.1f%%\n" % [float(result.get("player_win_rate", 0.0)) * 100.0, float(result.get("enemy_win_rate", 0.0)) * 100.0, float(result.get("draw_rate", 0.0)) * 100.0]
	text += "击退:%d 拉近:%d 自移:%d 崩势:%d\n" % [int(result.get("push_total", 0)), int(result.get("pull_total", 0)), int(result.get("self_move_total", 0)), int(result.get("break_total", 0))]
	text += "命中: 枪%d / 刀%d\n" % [int(result.get("player_hits", 0)), int(result.get("enemy_hits", 0))]
	text += "距离分布: %s" % _format_distance_ratio(result.get("distance_ratio", {}))
	return text
